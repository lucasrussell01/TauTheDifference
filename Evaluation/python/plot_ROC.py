...
# higgs vs total comparison
higgsall = pd.concat([higgs, total_bkg])
y_true = higgsall['class_label'].replace({11: 1, 12: 1, 2:0, 0:0})
y_pred = higgsall['pred_1']
w = higgsall['weight']

# calculate roc properties
fpr_hall, tpr_hall, thresholds_hall = metrics.roc_curve(y_true, y_pred, sample_weight=w)
...
plt.savefig(os.path.join(model_dir, 'ROC_higgs_vs_all.pdf'))
plt.close()


# Higgs VS Individual Backgrounds (one plot)
fix, ax = plt.subplots(figsize = (6,6))
ax.grid()
...
ax.text(0.01, 1.02, 'CMS', fontsize=20, transform=ax.transAxes, fontweight='bold', fontfamily='sans-serif')
ax.text(0.16, 1.02, 'Work in Progress', fontsize=16, transform=ax.transAxes, fontstyle='italic',fontfamily='sans-serif')
plt.savefig(os.path.join(model_dir, 'ROC_higgs_vs_indiv.pdf'))
plt.close()
