...
# File to draw from
base_path = '/vols/cms/lcr119/offline/HiggsCP/data/prod_feb/ShuffleMerge'
...
if channel == 'mt' or channel == 'et' or channel == 'tt':
    ...
    ewkz_tau = merged_df.loc[merged_df['process_id'] == 14]
    # lepton fakes
    DY_lep = merged_df.loc[(merged_df['process_id'] == 12) | (merged_df['process_id'] == 16)] # DY or EWK~
    # jet fakes
    EW = merged_df.loc[(merged_df['process_id'] == 43) | (merged_df['process_id'] == 53) | (merged_df['process_id'] == 16)]
    ...
    if not args.signal: # not just signal
        ...
        histo.add_bkg(ewkz_tau, "EWKZ_tau", weight=args.weight)
...
# Figure Saving
if not args.signal:
    fname = f"prod_feb/{args.era}_{channel}_{args.var}_{args.weight}.pdf"
else:
